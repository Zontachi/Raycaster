...
public float fPlayerX, fPlayerY, fPlayerAngle;
  ...
  void updatePlayer(){
    if (bPlayerAlive){ //Only allow actions when the player is alive
      ...
      if (iTimerMin == iTimerMax){
        background(0);
        caster.drawRays(60, channel2note/100 + 1, fPlayerX, fPlayerY, fPlayerAngle, channel1note/100 + 1, "Procedural", channel3note/100 + 1, false); //Draw white player vision rays with 1 bounce
        //caster.drawRays(1, 1, fPlayerX, fPlayerY, viewAngle(fPlayerX, fPlayerY, mouseX, mouseY), 0, "LightGreen" , 3, true); //Draw a single ray to indicate where the player is looking
      }
    }
    drawPlayer();
    remainingEnemies();
